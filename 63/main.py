...
#1
merged = rawNat.merge(rawPop[['Population','CountyCode']], left_index=True, right_index=True)
merged.fillna(np.mean(merged[labels], axis=0), inplace=True)
merged.reset_index().groupby('CountyCode').sum().apply(lambda row: row['LiveBirths']/row['Population']*1000 - row['Deceased']/row['Population']*1000, axis=1).to_csv('./dataOUT/Requirement_1.csv')
...
#3
rawHealth = pd.read_csv('./dataIN/DataSet_43.csv', index_col=0)
